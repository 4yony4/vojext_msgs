...
bool vojext_msgs__msg__bounding_boxes__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("vojext_msgs.msg._bounding_boxes.BoundingBoxes", full_classname_dest, 45) == 0);
  }
  vojext_msgs__msg__BoundingBoxes * ros_message = _ros_message;
  {  // header
  ...
  {  // bounding_boxes
    ...
    if (!vojext_msgs__msg__BoundingBox__Sequence__init(&(ros_message->bounding_boxes), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create vojext_msgs__msg__BoundingBox__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    vojext_msgs__msg__BoundingBox * dest = ros_message->bounding_boxes.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!vojext_msgs__msg__bounding_box__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * vojext_msgs__msg__bounding_boxes__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("vojext_msgs.msg._bounding_boxes");
  ...
  {  // bounding_boxes
    ...
    vojext_msgs__msg__BoundingBox * item;
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->bounding_boxes.data[i]);
      PyObject * pyitem = vojext_msgs__msg__bounding_box__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
